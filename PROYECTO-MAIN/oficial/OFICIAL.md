...
actor "Jefe Producción" as Jefe
boundary "IU_Ajustes" as UI
control "CTR_Inventario" as Ctrl
entity "CE_Catalogo_Items" as Cat
entity "CE_Kardex_Movimientos" as Kardex
entity "CE_Bitacora" as Bit
...
alt Ajuste Negativo
  ...
  alt Stock resultante < 0
    ...
    Kardex -> Bit : <<trigger>> registrar(accion_sql)
    activate Bit
    Bit --> Kardex : ok
    deactivate Bit
    deactivate Kardex
...
Cat -> Bit : <<trigger>> registrar(accion_sql)
activate Bit
Bit --> Cat : ok
deactivate Bit
deactivate Cat
...
actor "Jefe Producción" as Jefe
boundary "IU_Produccion" as UI
control "CTR_Produccion" as Ctrl
entity "CE_RecetaBOM" as BOM
entity "CE_DetalleReceta" as DetBOM
entity "CE_Kardex_Movimientos" as Kardex
entity "CE_OrdenProduccion" as Orden
entity "CE_Bitacora" as Bit
...
alt Todos los insumos disponibles
  ...
  Orden -> Bit : <<trigger>> registrar(accion_sql)
  activate Bit
  Bit --> Orden : ok
  deactivate Bit
  deactivate Orden
...
boundary "IU_Calidad" as UI
control "CTR_Calidad" as Ctrl
entity "CE_LoteProduccion" as Lote
entity "CE_FichaCalidad" as Ficha
entity "CE_Bitacora" as Bit
...
Ctrl -> Ctrl : evaluarVsEstandares(valores)
...
Ficha -> Bit : <<trigger>> registrar(accion_sql)
activate Bit
Bit --> Ficha : ok
deactivate Bit
deactivate Ficha
...
actor "Jefe Producción" as Jefe
boundary "IU_Parametros" as UI
control "CTR_Proceso" as Ctrl
entity "CE_OrdenProduccion" as Orden
entity "CE_ParametrosProceso" as Params
entity "CE_Bitacora" as Bit
...
Params -> Bit : <<trigger>> registrar(accion_sql)
activate Bit
Bit --> Params : ok
deactivate Bit
deactivate Params
...
actor "Jefe Producción" as Jefe
boundary "IU_CierreLote" as UI
control "CTR_Produccion" as Ctrl
entity "CE_OrdenProduccion" as Orden
entity "CE_LoteProduccion" as Lote
entity "CE_PresentacionesLote" as Pres
entity "CE_Bitacora" as Bit
...
Lote -> Bit : <<trigger>> registrar(accion_sql)
activate Bit
Bit --> Lote : ok
deactivate Bit
deactivate Lote
...
## 11.1 Herramientas de Desarrollo de la Aplicación WEB

Para la construcción física del Sistema Web de GRULAC S.R.L., se seleccionaron e implementaron las siguientes tecnologías de desarrollo moderno, optimizadas para el patrón BCE y la robustez transaccional:

*   **IDE y Workspace (Entorno de Trabajo Físico):** *Visual Studio Code v1.98* configurado como el entorno unificado de desarrollo. Con extensiones como *ESLint* para análisis estático, *Prettier* para formateo de código, e integración nativa de sintaxis *JSX/React* y *Next.js*. Bajo el enfoque de PUDS, permite la refactorización rápida y el mapeo ágil entre las clases de análisis lógico y sus componentes físicos.
*   **Entorno de Ejecución Frontend y Backend (Next.js):** *Next.js 15 (App Router)* actuando como la infraestructura unificada de desarrollo. Es el encargado de implementar físicamente el enrutamiento a través del sistema de carpetas físicas, las políticas de protección de rutas y la mediación segura de Server Components. React v19 gobierna las clases de frontera (`Boundary`), transformándolas en componentes web reactivos y encapsulados.
*   **Estilizado UI (Boundary Styling):** *Tailwind CSS v4* con un diseño premium *Dark Mode* en tonos `zinc-950` y `emerald-600` para las acciones primarias. Esto garantiza que la interfaz de usuario sea altamente intuitiva y adaptada a la iluminación de la planta de producción y del laboratorio, implementando controles visuales de semáforos (verde/rojo) para la validación fisicoquímica en caliente.
*   **Librerías de Componentes Reutilizables:** *Shadcn UI* (grillas editables, diálogos modales, tablas dinámicas y cards), *Lucide React* (iconografía técnica y comercial) y *Sonner* (notificaciones de eventos de stock, triage bioquímico y alertas del sistema).
*   **Backend Serverless y Capa de Control:** *Supabase Client* (`@supabase/ssr` y `@supabase/supabase-js`) actuando como la infraestructura intermedia y controladora. Es la encargada de resolver las peticiones físicas del navegador e interactuar con la base de datos a través de peticiones HTTP parametrizadas bajo el enfoque de seguridad JWT (JSON Web Tokens).
*   **Motor de Base de Datos y Persistencia (ACID Entities):** *PostgreSQL 15* físico alojado en la infraestructura cloud de AWS (RDS) gestionada por Supabase. Almacena físicamente el modelo relacional mapeado a partir del diagrama de clases de entidad de diseño. Garantiza la persistencia, integridad referencial y transaccionalidad ACID mediante constraints y la inmutabilidad de la bitácora mediante triggers implementados en funciones `PL/pgSQL`.
*   **Gestión de Configuración de Software (SCM):** *Git v2.45* integrado con *GitHub* para el control riguroso de versiones, la trazabilidad de los cambios físicos (código) respecto a los requisitos funcionales de cada ciclo, y el establecimiento de líneas base (baselines) estables al finalizar cada iteración.
*   **Entorno de Pruebas de APIs:** *Postman API* y la consola interactiva de Supabase para validar los endpoints de autenticación y los flujos DML relacionales antes de su ensamblado final en la interfaz de usuario Next.js.

---

## 11.2 Implementación de la Arquitectura del Sistema

La estructura física del software se organiza bajo una **Arquitectura en Capas** que distribuye físicamente los componentes de la aplicación de acuerdo con su responsabilidad de negocio. El siguiente diagrama de componentes ilustra la forma en que interactúan las capas físicas desde la interfaz de usuario en Next.js hasta la base de datos PostgreSQL:
...
package "React/Next.js SPA (Capa de Presentación)" {
  ...
  component "page.js (Kardex)" as COMP_Kardex
  component "page.js (Ajustes)" as COMP_Ajustes
  component "page.js (Recetas)" as COMP_Recetas
  component "page.js (Apertura)" as COMP_Apertura
  component "page.js (Parametros)" as COMP_Params
  component "page.js (Cierre)" as COMP_Cierre
  component "page.js (Ficha QA)" as COMP_Ficha
...
LAY_Side ..> COMP_Recetas
LAY_Side ..> COMP_Apertura
LAY_Side ..> COMP_Ficha

' Presentación a Controladores
COMP_Cat --> C_Inv
COMP_Kardex --> C_Inv
COMP_Ajustes --> C_Inv
COMP_Recetas --> C_Prod
COMP_Apertura --> C_Prod
COMP_Params --> C_Proc
COMP_Cierre --> C_Prod
COMP_Ficha --> C_Cal
COMP_Disp --> C_Cal
...
## 11.3 Implementación de la Arquitectura de Subsistemas

### Estructura de Directorios Física del Proyecto (Next.js App Router)

Para comprender la distribución de los componentes del software en el sistema de archivos real (`PROTOTIPO-GRULAC-CRM`), se detalla el árbol físico a continuación:

```
PROTOTIPO-GRULAC-CRM/
├── src/
│   ├── app/
│   │   ├── (dashboard)/
│   │   │   ├── (p2-gestion-usuario)/
│   │   │   │   ├── empleados/
│   │   │   │   │   └── page.js                 ← Formulario y grilla de empleados (RRHH)
│   │   │   │   ├── roles/
│   │   │   │   │   └── page.js                 ← Gestión de privilegios granulares y perfiles
│   │   │   │   └── perfil/
│   │   │   │       └── page.js                 ← Configuración personal de la cuenta
│   │   │   ├── (p3-gestion-inventario)/
│   │   │   │   ├── catalogo/
│   │   │   │   │   └── page.js                 ← Alta de insumos, materias primas y empaques
│   │   │   │   ├── kardex/
│   │   │   │   │   └── page.js                 ← Bitácora de movimientos IN/OUT de almacén
│   │   │   │   ├── ajustes/
│   │   │   │   │   └── page.js                 ← Registro justificado de mermas e incidentes
│   │   │   │   └── alertas/
│   │   │   │       └── page.js                 ← Configuración de stock de alerta paramétrico
│   │   │   ├── (p4-gestion-comercial)/
│   │   │   │   └── clientes/
│   │   │   │       └── page.js                 ← Cartera de clientes y directorios B2B
│   │   │   ├── (p5-gestion-proveedores-compras)/
│   │   │   │   ├── proveedores/
│   │   │   │   │   └── page.js                 ← Registro de ganaderos y colonias lecheras
│   │   │   │   └── compras/
│   │   │   │       └── page.js                 ← Abastecimiento de insumos químicos
│   │   │   ├── (p6-acopio-formulacion)/
│   │   │   │   ├── acopio/
│   │   │   │   │   └── page.js                 ← Recepción de cisterna y triage bioquímico
│   │   │   │   └── recetas/
│   │   │   │       └── page.js                 ← Definición de fórmulas BOM lácteas
│   │   │   ├── (p7-produccion-planta)/
│   │   │   │   └── produccion/
│   │   │   │       ├── apertura/
│   │   │   │       │   └── page.js             ← Inicio de orden, asignación Lote
│   │   │   │       ├── parametros/
│   │   │   │       │   └── page.js             ← Parámetros físicos (tina, temp, pH, Brix)
│   │   │   │       └── cierre/
│   │   │   │           └── page.js             ← Codificación, peso bruto y moldes
│   │   │   ├── (p8-control-calidad)/
│   │   │   │   └── calidad/
│   │   │   │       ├── ficha/
│   │   │   │       │   └── page.js             ← Registro fisicoquímico post-producción
│   │   │   │       ├── disposicion/
│   │   │   │       │   └── page.js             ← Retención en Cuarentena o Reproceso
│   │   │   │       └── liberacion/
│   │   │   │           └── page.js             ← Aprobación e ingreso formal a cámara de frío
│   │   │   ├── layout.js                       ← Guardia de seguridad RBAC y enrutador principal
│   │   │   └── inicio/
│   │   │       └── page.js                     ← Dashboard KPIs gerenciales
│   │   ├── layout.js                           ← Layout base HTML de la aplicación
│   │   ├── globals.css                         ← Estilos Tailwind CSS v4 e inyección de temas
│   │   ├── login/
│   │   │   └── page.js                         ← Login y verificación de accesos inicial
│   │   └── actualizar-contrasena/
│   │       └── page.js                         ← Gestión de restablecimiento de accesos
│   │   
│   ├── components/
│   │   ├── sidebar.js                          ← Sidebar lateral con navegación inteligente
│   │   ├── topbar.js                           ← Topbar de usuario con notificaciones
│   │   └── ui/                                 ← Componentes visuales Shadcn UI (Table, Dialog, etc.)
│   │
│   └── lib/
│       ├── controllers/                        ← Clase controladoras (Lógica de Negocio)
│       │   ├── CTR_Inventario.js               ← Lógica de catálogos, Kardex y mermas
│       │   ├── CTR_Produccion.js               ← Lógica de lotes, recetas BOM y cierres
│       │   ├── CTR_Proceso.js                  ← Lógica de parámetros y etapas en tina
│       │   └── CTR_Calidad.js                  ← Lógica de fichas laboratorios y retenciones
│       ├── supabase/
│       │   ├── client.js                       ← Conexión de navegador Supabase
│       │   └── server.js                       ← Conexión segura de backend Supabase
│       └── utils.js                            ← Funciones de formato de cadenas y fechas
```
